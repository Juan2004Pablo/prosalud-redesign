...
const images: Image[] = [
  ...
  { src: "https://www.sindicatoprosalud.com/portal/components/com_eventgallery/helpers/image.php?&width=1600&folder=HLM_241203&file=b00dd65f-4748-4003-bb3f-0e174881b8dc.jpg", alt: "Participantes en taller 2" },
  { src: "https://www.sindicatoprosalud.com/portal/components/com_eventgallery/helpers/image.php?&width=1600&folder=HLM_241203&file=d06ae58a-dc2a-48b0-8b20-45d8de68de57.jpg", alt: "Ejercicio de relajación 2" }
];
...
const GaleriaBienestarIntroSection: React.FC = () => {
  ...
  const columns: Image[][] = [
    images.slice(0,2), images.slice(2,4), [images[4]], [images[5]],
    [images[6]], [images[7]], [images[8]], images.slice(9,11), images.slice(11,13)
  ];

  const offsetClasses = [
    'translate-y-0','-translate-y-12','-translate-y-28','-translate-y-40',
    '-translate-y-32','-translate-y-40','-translate-y-28','-translate-y-12','translate-y-0'
  ];

   return (
    <section ref={sectionRef} id="galeria-bienestar-intro" className="relative bg-white rounded-2xl shadow-lg p-8 lg:p-16 overflow-hidden m-20">
      ...
      <div className="absolute inset-x-0 top-0 flex justify-center items-end gap-x-2 pointer-events-none">
        {columns.map((col, colIdx) => (
          <div
            key={colIdx}
            className={`${offsetClasses[colIdx]} transition-transform duration-700 ${mounted ? 'opacity-100' : 'opacity-0 translate-y-4'}`}>
            {col.map((img, imgIdx) => (
              <img
                key={imgIdx}
                src={img.src}
